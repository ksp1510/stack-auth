  ...
  el.innerHTML = `
    <div class="app">
      <section class="card wide" style="border-color:rgba(239,68,68,.35);background:rgba(239,68,68,.08);">
        <div class="title">stack-auth: SolidJS (JS) config error</div>
        <div class="sub">Missing required environment variables. Update <code>apps/solid-js/.env</code> and restart dev server.</div>
        ...
        <div class="small" style="margin-top:12px;">
          ...
          <ul style="margin:6px 0 0;padding-left:18px;">
            <li>Callback: <code>http://localhost:5184/callback</code></li>
            <li>Web origin: <code>http://localhost:5184</code></li>
            <li>Logout: <code>http://localhost:5184/</code></li>
          </ul>
        </div>
      </section>
    </div>
  ...
  el.innerHTML = `
    <div class="app">
      <section class="card" style="border-color:rgba(245,158,11,.35);background:rgba(245,158,11,.08);">
        <div class="title">stack-auth: SolidJS (JS) init error</div>
...
(async () => {
  ...
  if (err) {
    ...
    navigate("/"); // <-- important: update Solid router signal
  }
